//------------------------------------------------------------------
// Joel Giedt   ADDED: QPropWRandWallDiluted
// Dean Howarth ADDED: QPropWRandWallSrcDilutedSmeared
//                     QPropWRandWallSrcSmeared
//                     QPropWMomSrcSmeared
//                     Stout GKLS routines
//                     Z_2 X Z_2 random source
//------------------------------------------------------------------
...
//Begin QUDA_CPS
//------------------------------------------------------------------
// Quark Propagator (Wilson type) with momentum wall source
//------------------------------------------------------------------
QPropWMom::QPropWMom(Lattice& lat, CommonArg* c_arg): 
  QPropW(lat, c_arg) {
  
  char *fname = "QPropWMom(L&, ComArg*)";
  cname = "QPropWMom";
  VRB.Func(cname, fname);
...
// Allocate and Delete space for the momentum source
void QPropWMom::AllocateMsrc() {

  char *fname = "AllocateMsrc()";
  VRB.Func(cname, fname);

  if (msrc == NULL) {
	int msrc_size = 2 * GJP.VolNodeSites();
	msrc = (Float*)smalloc(msrc_size * sizeof(Float));
	if (msrc == 0) ERR.Pointer(cname, fname, "msrc");
	VRB.Smalloc(cname, fname, "msrc", msrc, msrc_size * sizeof(Float));
  }
}
void QPropWMom::DeleteMsrc() {

  char *fname = "DeleteMsrc()";
  VRB.Func(cname, fname);
  
  if (msrc != NULL) {
    VRB.Sfree(cname, fname, "msrc", msrc);
    sfree(msrc);
    msrc = NULL;
  }
}

QPropWMom::QPropWMom(Lattice& lat,  QPropWArg* arg, 
		     int* p, CommonArg* c_arg): 
  QPropW(lat, arg, c_arg), mom(p) {
...
QPropWMom::QPropWMom(const QPropWMom& rhs) : QPropW(rhs), msrc(NULL) {
...
Complex& QPropWMom::mom_src(int i) const   
{ return ((Complex*)msrc)[i]; }

QPropWMom& QPropWMom::operator=(const QPropWMom& rhs) {

  char *fname = "operator=(const QPropWMom& rhs)";
  VRB.Func(cname, fname);
...
QPropWMomSrc::QPropWMomSrc(Lattice& lat, QPropWArg* arg, int* p,
			   CommonArg* c_arg) : 
  QPropWMom(lat, arg, p, c_arg) , mom(p) {
...
void QPropWMomSrc::SetSource(FermionVectorTp& src, int spin, int color) {
  ...
  //Copy source to msrc for use by mom_src
  if(spin == 0 && color == 0 ) {
    Site s;
    for(s.Begin(); s.End(); s.nextSite()) {
      if( qp_arg.t == s.physT()) {// continue only if physical
	                          // time is qp_arg.t
	msrc[2*s.Index()  ] = src[2*(color + COLORS*(spin + 4*s.Index()))  ];
	msrc[2*s.Index()+1] = src[2*(color + COLORS*(spin + 4*s.Index()))+1];
      }
      else msrc[2*s.Index()  ] = msrc[2*s.Index()+1] = 0.0;
    }
  }
  if (GFixedSrc()) src.GFWallSource(AlgLattice(), spin, 3, qp_arg.t);
    
}

QPropWMomSrcSmeared::QPropWMomSrcSmeared(Lattice& lat,  QPropWArg* arg, 
					 int* p, QPropWGaussArg* g_arg, CommonArg* c_arg) : 
  QPropWMom(lat, arg, p, c_arg), mom(p), gauss_arg(*g_arg) {
 
  char *fname = "QPropWMomSrcSmeared(L&, QPropWArg*, p, GaussArg*, ComArg*)";
  cname = "QPropWMomSrc";
  VRB.Func(cname, fname);
...
void QPropWMomSrcSmeared::SetSource(FermionVectorTp& src, int spin, int color) {
  char *fname = "SetSource()";
  VRB.Func(cname, fname);
...
QPropWMomSrcDilutedSmeared::QPropWMomSrcDilutedSmeared(Lattice& lat,  QPropWArg* arg, 
						       int* p, QPropWGaussArg* g_arg,
						       CommonArg* c_arg, int* t_L, int t_index) : 
  QPropWMom(lat, arg, p, c_arg), mom(p), gauss_arg(*g_arg) {
  
  char *fname = "QPropWMomSrcDilutedSmeared(L&, QPropWArg*, p, GaussArg*, ComArg*, *L, t)";
  cname = "QPropWMomSrc";
  VRB.Func(cname, fname);
  
  L[0]=t_L[0]; L[1]=t_L[1]; L[2]=t_L[2];
  index=t_index;
  // Sanity checks:
  int subvol=L[0]*L[1]*L[2];
  if (index < 0 || index >= subvol) ERR.General(cname,fname,"bad value of dilution index\n");
  if ((GJP.XnodeSites()*GJP.Xnodes()) % L[0] || (GJP.YnodeSites()*GJP.Ynodes()) % L[1] 
                  || (GJP.ZnodeSites()*GJP.Znodes()) % L[2])
    ERR.General(cname,fname,"volume not divisible by dilution subvolume\n");
  
  Run();
}

void QPropWMomSrcDilutedSmeared::SetSource(FermionVectorTp& src, int spin, int color) {

  char *fname = "SetSource()"; 
  VRB.Func(cname, fname);

  if (msrc==NULL) {//need random numbers may implemented later
    ERR.General(cname,fname,"No momentum source found!\n") ;
  }

  // Calculate the momentum phases for the source.
  src.ZeroSource();
  src.SetMomSource(color, spin, qp_arg.t, mom);
  //Copy source to msrc for use by mom_src
  if(spin == 0 && color == 0 ) {
	...
	msrc[2*s.Index()+1] = src[2*(color + COLORS*(spin + 4*s.Index()))+1];
      ...
      //printf("1: %.6f %.6f\n", msrc[2*s.Index()], msrc[2*s.Index()+1]);
    }
  }
  // Zero out msrc in appropriate places so that we can form the propagator.
  int i;
  int subX = index % L[0];
  int subY = (index/L[0]) % L[1];
  int subZ = index/(L[0]*L[1]);
  for (i=0;i<GJP.VolNodeSites();i++) {
    int localX= i % GJP.XnodeSites();
    int localY= (i/GJP.XnodeSites()) % GJP.YnodeSites();
    int localZ= (i/(GJP.XnodeSites()*GJP.YnodeSites())) % GJP.ZnodeSites();
    int globalX= localX+GJP.XnodeSites()*GJP.XnodeCoor();
    int globalY= localY+GJP.YnodeSites()*GJP.YnodeCoor();
    int globalZ= localZ+GJP.ZnodeSites()*GJP.ZnodeCoor();
    int isubX= globalX % L[0];
    int isubY= globalY % L[1];
    int isubZ= globalZ % L[2];
    if (isubX != subX || isubY != subY || isubZ != subZ) {
      msrc[2*i]=msrc[2*i+1]=0.0;
    }
    //printf("2: %.6f %.6f\n", msrc[2*i], msrc[2*i+1]);
  }
  // Reset the momentum source using the dilution.
  src.ZeroSource();  
  src.SetWallSource(color, spin, qp_arg.t, msrc);
  if (GFixedSrc()) src.GFWallSource(AlgLattice(), spin, 3, qp_arg.t);
  DoLinkSmear(gauss_arg);  // YA: smear link if needed
...
// BEGIN Joel Giedt //
//////////////////////
// This class will create spatial dilution of sub-volumes
// on the time slice.
//------------------------------------------------------------------
// Quark Propagator (Wilson type) with Random Wall Source
//------------------------------------------------------------------
QPropWRandWallSrcDiluted::QPropWRandWallSrcDiluted(Lattice& lat,  QPropWArg* arg, QPropWRandArg *r_arg, CommonArg* c_arg, int t_L[], int t_index) :
  QPropWRand(lat, arg, r_arg, c_arg) {
  
  char *fname = "QPropWRandWallSrcDiluted(L&, ComArg*)";
  cname = "QPropWRandWallSrcDiluted";
  VRB.Func(cname, fname);
  L[0]=t_L[0]; L[1]=t_L[1]; L[2]=t_L[2];
  index=t_index;
  // Sanity checks:
...
void QPropWRandWallSrcDiluted::SetSource(FermionVectorTp& src, int spin, int color) {
  ...
  // Zero out rsrc in appropriate places so that we can form the propagator.
  int i;
  int subX = index % L[0];
  int subY = (index/L[0]) % L[1];
  int subZ = index/(L[0]*L[1]);
...
// This class will take a random wall source and perform Gaussian smearing
// on the sources.
//------------------------------------------------------------------
// Quark Propagator (Wilson type) with Smeared, Random Wall Source
//------------------------------------------------------------------
QPropWRandWallSrcSmeared::QPropWRandWallSrcSmeared(Lattice& lat,  QPropWArg* arg, QPropWRandArg *r_arg, CommonArg* c_arg, QPropWGaussArg *g_arg) :
  QPropWRand(lat, arg, r_arg, c_arg), gauss_arg(*g_arg)
{
  
  char *fname = "QPropWRandWallSrcSmeared(L&, QPropWarg*, QPropWRandArg*, ComArg*";
  cname = "QPropWRandWallSrcSmeared";
  VRB.Func(cname, fname);
  Run();
}

void QPropWRandWallSrcSmeared::SetSource(FermionVectorTp& src, int spin, int color) {
...
// This class will take a random wall source, perform spatial dilution, and
// perform Gaussian smearing of the sources.
//-------------------------------------------------------------------------
// Quark Propagator (Wilson type) with Diluted, Smeared, Random Wall Source
//-------------------------------------------------------------------------
QPropWRandWallSrcDilutedSmeared::QPropWRandWallSrcDilutedSmeared(Lattice& lat,  QPropWArg* arg, QPropWRandArg* r_arg, CommonArg* c_arg, QPropWGaussArg *g_arg, int t_L[], int t_index) :
  QPropWRand(lat, arg, r_arg, c_arg), gauss_arg(*g_arg)//Inherit this class to have access to a random wall source.
{
  char *fname = "QPropWRandWallSrcDilutedSmeared(L&, QPropWArg*, QPropQWRandArg*, ComArg*)";
  cname = "QPropWRandWallSrcDilutedSmeared";  
  VRB.Func(cname, fname);

  ///////////////////////////////
  // Use Joel's dilution code. //
  ///////////////////////////////
  L[0]=t_L[0]; L[1]=t_L[1]; L[2]=t_L[2];
  index=t_index;
  // Sanity checks:
  int subvol=L[0]*L[1]*L[2];
  if (index < 0 || index >= subvol) ERR.General(cname,fname,"bad value of dilution index\n");
  if ((GJP.XnodeSites()*GJP.Xnodes()) % L[0] || (GJP.YnodeSites()*GJP.Ynodes()) % L[1] || (GJP.ZnodeSites()*GJP.Znodes()) % L[2])
    ERR.General(cname,fname,"volume not divisible by dilution subvolume\n");
  // get the propagator
  Run();
}

//Set diluted Gaussian source
void QPropWRandWallSrcDilutedSmeared::SetSource(FermionVectorTp& src, int spin, int color)
{
  char *fname = "SetSource()";
  VRB.Func(cname, fname);
 
  // Need to zero out rsrc in appropriate places so that we can form the propagator.
  int i;
  int subX = index % L[0];
  int subY = (index/L[0]) % L[1];
  int subZ = index/(L[0]*L[1]);
  ...
  for (i=0;i<GJP.VolNodeSites();i++) {
    ...
    int isubZ= globalZ % L[2];
    //Zero out the unwated sources.
    if (isubX != subX || isubY != subY || isubZ != subZ) rsrc[2*i]=rsrc[2*i+1]=0.0;
  }
  src.SetWallSource(color, spin, qp_arg.t, rsrc);
  DoLinkSmear(gauss_arg);  // YA: smear link if needed    
  src.GaussianSmearVector(AlgLattice(),spin, gauss_arg.gauss_N, gauss_arg.gauss_W, qp_arg.t);  
