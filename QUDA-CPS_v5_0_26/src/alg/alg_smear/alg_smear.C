      ...
      for (mu=0;mu<4;++mu)
        {
	      ...
	      //DMH: For stout smearing, this projection step ought
	      //     not be necessary. In the interests of optimisation,
	      //     projection is omitted for stout by hardcoding the 
	      //     value bool_su3_proj = -1.      
...
//Begin QUDA_CPS
AlgStoutSmear::AlgStoutSmear(Lattice&     lat,
			     CommonArg*   ca ,
			     StoutSmearArg* asa,
			     int in_bool_su3_proj):
  //DMH Hardcode SU3(3) projection to -1. 
  AlgSmear(lat,ca,-1),
  cname("AlgStoutSmear")
{
  c = asa->coef;
...
/*!
  ...
  the smearing coefficients are written to the file.
*/
void AlgStoutSmear::run()
{
  if(common_arg->filename != 0){
    FILE* f = Fopen(common_arg->filename, "a");
    if(!f) ERR.FileA(cname, "run", common_arg->filename);
    Fprintf(f,"AlgStoutSmear: rho = %e ",c);
    Fclose(f);
  }
  AlgSmear::run();
}

static Float inv3 = 1.0/3.0;

void AlgStoutSmear::smear_link(Matrix& link,
			       int*     pos,
			       int       mu)
{
  Lattice& lattice(AlgLattice());
  ...
  //This function gets stap = C_{mu,nu} i.e., the staple of length 3,
  //
  // |- > -|    
  // ^     v    
  // |     |    
  ...
  //For now, we have set rho=diag(0,c,c,c)
  Matrix linkDAG;
  linkDAG.Dagger(link);
  Matrix Omega;
  Omega.DotMEqual(stap,linkDAG);
  Omega *= c;

  //Compute \Q_{mu} = i/2[Omega_{mu}^dag - Omega_{mu} - 1/3 Tr(Omega_{mu}^dag - Omega_{mu})]
  Matrix OmegaDag;
  OmegaDag.Dagger(Omega);
  Matrix Omega_diff;
  Omega_diff = OmegaDag;
  Omega_diff -= Omega;
  ...
  Matrix Q;
  Q = Omega_diff;
  Complex Omega_diff_Tr = Omega_diff.Tr();

  //Omega_diff_Tr *= 1.0/3.0;
  Omega_diff_Tr *= inv3;
  Matrix ODT; ODT = Omega_diff_Tr;
  Q -= ODT;
  Complex imag; imag.real(0.0); imag.imag(0.5);
  Matrix temp1; temp1 = imag;
  Matrix temp2;
  temp2.DotMEqual(temp1,Q);
  //Q is now defined.
  Q = temp2;
  ...
  Complex Q_tr;
  Q_tr = Q.Tr();
  cout<<"Tr Qre:"<<Q_tr.real()<<" Qim:"<<Q_tr.imag()<<endl;

  //Test for hermiticiy.
  temp1.Dagger(Q);
  Matrix Q_diff = Q;
  Q_diff -= temp1; //This should be the zero matrix. Test by Tr(Q_diff^2);
  temp2.DotMEqual(Q_diff,Q_diff);
  Q_tr = temp2.Tr();
  cout<<"Tr q_diffre:"<<Q_tr.real()<<" q_diffim:"<<Q_tr.imag()<<endl;
  */
  ...
  /*
  //TEST FOR UNITARITY
  Matrix DAG; DAG.Dagger(Omega);
  temp.DotMEqual(Omega,DAG);
  cout<<"Tr eedag "<<temp.ReTr()<<endl;
  */

  //Use stap as temp to hold exp_iQ * U
  stap.DotMEqual(Q,link);
  link = stap;
  
  //***DEBUGGING***//
  /*
  //TEST FOR UNITARITY
  Matrix DAG; DAG.Dagger(stap);
  temp1.DotMEqual(link,DAG);
  Complex UN_test; UN_test = temp1.Tr();
  cout<<"Tr UUdag re:"<<UN_test.real()<<"   im:"<<UN_test.imag()<<endl<<endl;
  */
}
//end QUDA_CPS
