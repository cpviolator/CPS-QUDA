...
void AlgRotateGauge::run()
{
  ...
  //Begin QUDA_CPS
  // Remove this exception to allow for other gaueg rotations.
      ...
      for(int z=0; z<NZ; z++)
	{
	  for(int y=0; y<NY; y++)
	      ...
	      for(int x=0; x<NX; x++) 
		{
		  ...
		  //Begin QUDA_CPS 
		  // The offset for Landau gauge is simple as there
		  // is only a single pointer to one hyperplane.
		  if ( lat.FixGaugeKind() == FIX_GAUGE_LANDAU ) {
		  ...
		  // The offset for Coulomb must be done in two steps
		  // as there are multiple pointers to different
		  // hyperplanes.		  
		  ...
		  // The local position of the gl matrix will 
		  // always be on the curtent hyperplane.
		  if ( lat.FixGaugeKind() == FIX_GAUGE_COULOMB_T ) {
		    m_offset = lat.FixGaugePtr()[t] + offset_st;
		  }
		  // if ( lat.FixGaugeKind() == FIX_GAUGE_COULOMB_Z ) {
		  //   m_offset = lat.FixGaugePtr()[z] + offset_st;
		  // }
		  // if ( lat.FixGaugeKind() == FIX_GAUGE_COULOMB_Y ) {
		  //   m_offset = lat.FixGaugePtr()[y] + offset_st;
		  // }
		  // if ( lat.FixGaugeKind() == FIX_GAUGE_COULOMB_X ) {
		  //   m_offset = lat.FixGaugePtr()[x] + offset_st;
		  // }
		  //End QUDA_CPS
		  for(int nu=0; nu<4; nu ++)
			...
			mtmp = GetMat(  m_offset,
		      ...
		      if ( lat.FixGaugeKind() == FIX_GAUGE_COULOMB_T ) {
			if(nu == 3) {
			  if(t < (NT-1) ) m_offset = lat.FixGaugePtr()[t+1] + offset_st;
			  else m_offset = lat.FixGaugePtr()[0] + offset_st;
			  mtmp = *m_offset;
			}
			else {
			  mtmp = GetMat(  m_offset,
		      ...
		      //if( x==0 && y==0 && z==0 && t==0) fprintf(stdout, "%d %d %d %d %d %f %f\n", x,y,z,t,nu, mtmp.ReTr(), mtmp.ErrorSU3());
		      //End QUDA_CPS
		      gr.Dagger( mtmp );
